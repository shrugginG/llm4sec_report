...
<img src="./images/arxiv-logo.svg" alt="logo" width="80" height="80" style="position: absolute; top: 20px; right: 20px; z-index: 10;">

## **Large Language Models for Cyber Security: A Systematic Literature Review**

[arXiv:2405.04760](https://arxiv.org/abs/2405.04760)
...
<p class="small-text">本文对LLMs在网络安全中的应用（LLM4Security）进行了全面的文献回顾。通过收集超过4万篇相关论文，系统分析了来自顶级安全和软件工程会议的185篇论文，旨在提供一个关于LLMs如何解决网络安全领域各种问题的整体视角。</p>
...
**主要发现**:
* LLMs正被应用于更广泛的网络安全研究
* 不同LLM架构（如Encoder-only, Encoder-decoder, Decoder-only）的应用趋势
...
* 筛选了185篇高质量论文进行详细分析
* 提出了四个关键研究问题，从模型类型、安全研究、领域特定技术和数据处理等多个维度进行分析
...
* **研究问题 (RQs)**:
    1.  **RQ1**: LLM辅助了哪些类型的安全研究？
    2.  **RQ2**: 哪些LLMs被应用于安全研究？
    3.  **RQ3**: 使用了哪些领域特定技术来使LLMs适应安全研究？
    4.  **RQ4**: 在将LLMs应用于不同安全研究时，数据收集和预处理有何差异？
...
**RQ1: LLM辅助了哪些类型的安全研究？**
* **关注点**: LLMs已应用的安全研究的范围和性质。
* **目标**: 分类并理解LLMs正用于解决的安全挑战的广度，突出模型在各种安全维度上的适应性和有效性。
* **方法**: 根据不同安全领域对先前研究进行分类，并详细介绍各领域中使用LLMs的各种安全研究。

**RQ2: 哪些LLMs被用于支持安全研究？**
* **关注点**: 已在安全研究中使用的具体LLMs清单。
...
**RQ3: 使用了哪些特定技术来使LLMs适应安全研究？**
* **关注点**: 用于微调或调整LLMs以适应安全研究的具体方法和技术。
* **目标**: 理解这些技术，为增强LLMs在专门任务中有效性的定制过程提供有价值的见解。
* **方法**: 通过分析论文中采用的领域特定技术，揭示这些技术与特定安全研究之间的内在和特定联系。

**RQ4: 数据收集和预处理有何差异？**
* **关注点**: LLMs与特定任务所用数据之间相关性。
* **目标**: 从数据收集和数据预处理两个维度揭示在安全研究中应用LLMs时因数据产生的挑战。
* **方法**: 总结数据、安全研究和LLMs之间的内在关系。
...
1.  **识别相关会议和数据库**:
    ...
    * **arXiv**
    * 各大论文数据库: ACM Digital Library, IEEE Xplore, Science Direct, Web of Science, Springer, Wiley, arXiv。
2.  **建立QGS**: 手动筛选118篇LLM4Sec相关论文。
3.  **定义搜索关键词**: 基于QGS论文标题摘要进行词频分析。
    * LLM相关: Large Language Model, LLM, GPT, ChatGPT, T5, Llama等。
    * 安全研究相关: Cyber Security, Vulnerability Detection, Malware Analysis, Phishing等。
...
通过精心设计的提示词来引导LLM完成特定安全研究，无需修改模型参数。包括零样本学习、少样本学习和思维链推理等策略。这种方法能够直接利用预训练模型的知识，快速适应各种安全场景，是最直接且成本最低的LLM应用方式。
...
在预训练LLM基础上，使用特定安全研究的数据集进一步训练模型参数，使其更好地适应安全领域。包括全参数微调、参数高效微调(PEFT)和强化学习等策略。虽然需要额外的计算资源，但能显著提升模型在特定安全研究上的性能表现。
...
通过集成外部工具、知识库或其他模型来增强LLM的安全研究处理能力。包括检索增强生成(RAG)、工具调用、多模型协作等技术。这种方法能够弥补LLM在领域知识、实时信息和专业工具方面的不足，提供更全面可靠的安全解决方案。
...
* **对数据特征有限的安全研究尤其有益**: 当常规数据集（网络威胁报告、社交媒体有害内容、代码漏洞数据集等）受限或细节不足时，提示工程至关重要。
* **应用示例**:
    * **网络威胁分析**: 构建包含当前网络安全态势的提示，使LLM以零样本学习方式直接从流特征中学习，从非结构化数据中提取结构化网络威胁情报。
    * **程序Fuzzing**: 将多个独立测试用例集成到提示中，辅助LLM学习测试用例特征并通过少样本学习生成新测试用例。
    * **渗透测试** 和 **硬件漏洞验证**: 涉及多步骤和严格逻辑推理的任务，可利用思维链 (CoT) 指导提示定制，辅助LLM进行过程推理并自主分步完成任务。
* **普遍性**: 在LLM4Security中，几乎所有安全研究都涉及提示工程，突显了提示的不可或缺作用。

结论：提示工程在增强LLM针对特定安全研究的性能方面起着关键作用，有助于开发自动化安全研究解决方案。
...
* **基于强化学习(RL)的策略**: 基于强化学习的微调通过设计任务相关的奖励函数（如模糊测试中的绕过成功率、程序修复中的测试通过率），让LLM根据输出结果的反馈不断调整生成策略，从而学会产生更有效的安全研究解决方案。使LLM更符合特定安全目标。
* **挑战**:
    * 高质量、大规模、代表性标记数据集稀缺。
    * 计算资源需求（尤其全参数微调）。
    * 过拟合风险，泛化能力受损。
...
4.  **任务自适应训练 (Task-adaptive Training)**: 采用对比学习或蒸馏等训练策略，增强LLM对复杂安全研究的适应性。
...
外部增强技术显著提升了LLM在各种安全研究中的有效性，是解决LLM领域知识缺陷和任务对齐问题的有潜力途径。
...
4.  **代码表示**: 将片段转换为适合LLM处理的格式，通常涉及安全研究的词元化。
...
在网络安全研究中应用LLM的一个常见挑战是缺乏高质量的标记数据。漏洞、恶意软件、特定攻击类型等数据集在数量和多样性上难以充分获取。为解决此瓶颈，许多研究探索利用LLM自身的生成能力进行数据增强。目标是提升数据集大小、多样性和平衡性，从而提高在增强数据上训练（通常通过微调）的安全模型的鲁棒性和泛化性能。
...
* 为确保LLM稳健泛化，需在各种安全研究数据集上进行细致微调、验证和持续反馈循环。否则有模型过拟合风险，限制其在多样真实场景中的效用。
...
* **LLM的外部检索与工具**:
    ...
    * 可行方法：使其与外部世界交互，获取知识以提高生成安全研究解决方案的真实性和合理性。
...
* **扩展LLM应用**:
    * LLM在软件和系统安全领域受关注多。
    * 在硬件安全、区块链安全等领域应用仍有限，某些特定安全研究未被探索。
